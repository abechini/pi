...
public interface AppoitementRepository  extends JpaRepository<Appoitement,Long> {
    ...
    @Query("select a from Appoitement a where a.agent.id = :agentId and a.start >=:dayStart and  a.start <=:dayEnd")
    List<Appoitement> findAllAppointmentWithStartInPeroid(@Param("agentId") Long agentId, @Param("dayStart") LocalDateTime startPeroid, @Param("dayEnd") LocalDateTime endPeroid);
     Appoitement  save(Appoitement appoitement) ;
